...
const CobraStretchMenengah = ({ route }) => {
    ...
    try {
      if (timer === 20) {
        const { sound: move } = await Audio.Sound.createAsync(
          require("../../../../../assets/sounds/perut_menengah/cobra_stretch.mp3")
        );
        setSound(move);
  ...
  useEffect(() => {
    ...
    if (timer === 0) {
      ...
      navigation.navigate("HasilPerutMenengah", {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.containerGift}>
        <Image
          source={require("../../../../../assets/gif/perut_menengah/cobra_stretch.gif")}
          style={styles.gift}
        />
      </View>
...
export default CobraStretchMenengah;
